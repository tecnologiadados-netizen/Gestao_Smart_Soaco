...
export const SORT_DEFAULT_RESSUP_NAO_ALMOX: SortLevel[] = [
  { id: 'coleta', dir: 'asc' },
...
export function getOrderLabelsForRessupNaoAlmoxCol(columnId: string): { asc: string; desc: string } {
...
export function isRessupNaoAlmoxColData(columnId: string): boolean {
  return COLUNAS_DATA.has(columnId);
}

export function isRessupNaoAlmoxColNumeric(columnId: string): boolean {
