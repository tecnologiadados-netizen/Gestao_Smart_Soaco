/** Percentual de variação (markup) por grupo de produto — Faturamento Indireto Líquido (MKP). */
export type DreMkpVariacaoRow = {
  grupoProduto: string;
  variacao: number;
};

export const DRE_MKP_VARIACOES: DreMkpVariacaoRow[] = [
...
const ALIASES_GRUPO: Record<string, string> = {
  'balcao frigorifico sem tenda': 'Balcão frigorífico sem tendal',
  'material comprado': 'Material comprado',
};

/** Aliases Nomus → linha da DRE (Receita de vendas de produtos). */
...
export function nomeGrupoProdutoDre(nomeNomus: string): string {
  ...
  return ALIASES_GRUPO_NOMUS[chave] ?? ALIASES_GRUPO[chave] ?? nomeNomus.trim();
}

export function variacaoMkpPorGrupo(nome: string): number | null {
  const canonico = nomeGrupoProdutoDre(nome);
  const row = DRE_MKP_VARIACOES.find((r) => normalizarGrupoProduto(r.grupoProduto) === normalizarGrupoProduto(canonico));
  return row?.variacao ?? null;
}

/** Aplica markup sobre o valor bruto: líquido = bruto × (1 + variação%). */
export function calcularValorComMkp(bruto: number, variacaoPct: number): number {
  if (!Number.isFinite(bruto) || bruto === 0) return 0;
  if (!Number.isFinite(variacaoPct) || variacaoPct === 0) return bruto;
  return bruto * (1 + variacaoPct / 100);
}

/** Faturamento indireto: (valorUnitario − valorUnitario×MKP/100) × qtde */
...
export function formatarVariacaoMkp(variacao: number): string {
