  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const tipo = tipos.find((t) => t.code === 'faturamento_diario');
        if (!tipo) return;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <p className="text-sm text-slate-600 dark:text-slate-400">
        A mensagem de faturamento do dia é enviada automaticamente
        {horariosAgendamento ? ` às ${horariosAgendamento}` : ' nos horários configurados'} para os destinatários
        configurados em{' '}
      ...
      {semDestinatarios && (
        <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 px-3 py-2 text-sm text-amber-800 dark:text-amber-200">
          Nenhum destinatário com telefone configurado para &quot;Faturamento diário&quot;. Configure em{' '}
