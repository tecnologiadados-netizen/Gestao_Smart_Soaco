import type {
  EmailNotificacaoTipo,
  EmailNotificacaoTipoSave,
  UsuarioDestinatarioEmail,
} from '../../../api/integracaoEmail';
import SmsHorariosAgendamento from '../sms/SmsHorariosAgendamento';
...
type Props = {
  ...
  idx: number;
  saved: EmailNotificacaoTipo | undefined;
  podeEditar: boolean;
  isExpanded: boolean;
  destIds: number[];
  usuarios: UsuarioDestinatarioEmail[];
  filtroUsuario: string;
  usuariosFiltrados: UsuarioDestinatarioEmail[];
  savingDest: number | null;
  onUpdate: (patch: Partial<EmailNotificacaoTipoSave>) => void;
  onToggleExpand: () => void;
  onTest: () => void;
  onPreview: () => void;
...
export default function EmailTipoCard({
  tipo: t,
  idx,
  saved,
  podeEditar,
  ...
  const destSemEmail = destIds.filter((id) => {
    const u = usuarios.find((x) => x.id === id);
    return u && !u.email?.trim();
  }).length;
      ...
      <div className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                onUpdate({ code: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') })
              }
              placeholder="ex.: financeiro_credito_pedido_atraso"
            />
          </div>
          <div>
        ...
        {t.builderCode && (
          <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
            Builder: <code className="font-mono">{t.builderCode}</code>
          </p>
        )}
        ...
        {tipoId != null && isExpanded && (
          <div className="mt-4 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-900/20 p-4 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  Usuários que receberão este e-mail.
                  {destSemEmail > 0 && (
                    <span className="text-amber-600 dark:text-amber-400 ml-1">
                      {destSemEmail} selecionado(s) sem e-mail cadastrado.
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="max-h-56 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-0 divide-y sm:divide-y-0 sm:gap-px sm:bg-slate-200 dark:sm:bg-slate-600">
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span
                          className={`block text-xs truncate ${semEmail ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500 dark:text-slate-400'}`}
                        >
                          {u.email || 'Sem e-mail'}
                        </span>
                      </span>
