...
/** Cores do manual da marca Só Aço (identidade visual). */
const BRAND = {
  bg: '#000000',
...
export default function Login() {
  ...
  const PING_INITIAL_DELAY_MS = 1200;
  useEffect(() => {
    const t = setTimeout(() => {
    ...
    >
      ...
      >
        <div className="flex flex-col items-center mb-6">
          <LogoSoAco className="w-full max-w-[300px] h-auto mb-5" />
          <h1
        ...
        <form onSubmit={handleSubmit}>
          ...
          {serverOnline === false && (
            <p className="text-accent-500 text-sm mb-4 text-center">
              Servidor offline. Na pasta raiz execute: <code className="bg-black/20 px-1 rounded">npm run dev</code>
            </p>
          )}
          {error && (
            <p className="text-accent-500 text-sm mb-4 text-center">{error}</p>
          )}
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 rounded-lg text-white font-medium transition disabled:opacity-50 hover:opacity-95 focus:ring-2 focus:ring-accent-500"
            style={{
              background: BRAND.primary,
            }}
            onMouseEnter={(e) => { if (!loading) e.currentTarget.style.background = BRAND.primaryHover; }}
