...
function contagemItensPorFaixa(itens: CoberturaEstoqueLinha[]): Map<BarraFirme, number> {
  ...
  for (const i of itens) {
    if (i.faixaFirme == null) continue;
    acc.set(i.faixaFirme, (acc.get(i.faixaFirme) ?? 0) + 1);
  }
  return acc;
}
...
export default function CoberturaEstoquePage() {
  ...
  const itensPorFaixa = useMemo(() => {
    ...
    return painel.itens.filter((i) => {
      if (barraAtiva) return i.faixaFirme === barraAtiva;
  ...
  const capitalPorFaixa = useMemo(() => {
    ...
    for (const i of itensRecortePainel) {
      if (i.faixaFirme == null || i.valorFirme == null) continue;
      acc.set(i.faixaFirme, (acc.get(i.faixaFirme) ?? 0) + i.valorFirme);
    }
    return acc;
  }, [itensRecortePainel]);
          ...
          <>
                  ...
                  <div className="mb-2 flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Soma de (estoque − empenho) × preço unitário por faixa. Respeita o recorte de KPI/barras acima.
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  >
                    {BARRAS_FIRME_ORDEM.map((faixa) => {
                      ...
                      const temCapital = itensRecortePainel.some(
                        (i) => i.faixaFirme === faixa && i.valorFirme != null
                      );
            ...
            <section className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900/40">
              ...
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                Barras de cobertura (só CM &gt; 0): REPOR (&lt; 0 e 0–0,5), VIGIAR (0,5–1), OPERAÇÃO NORMAL (1–3), CAPITAL
                PARADO (&gt; 3). Status Ruptura/Aguardando PC vêm do empenho vs estoque, não só da barra.
              </p>
