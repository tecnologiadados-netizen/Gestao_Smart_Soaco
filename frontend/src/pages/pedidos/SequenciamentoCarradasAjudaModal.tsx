...
const SECOES: SecaoAjuda[] = [
  ...
  },
  {
    ...
    comoLe:
      'Clique no semáforo/material para abrir o horizonte: saldo início, consumo, entrada de PC e saldo projetado dia a dia. Verde/amarelo/vermelho resumem risco no período analisado. No modal Materiais do dia, a coluna Entrada PC mostra uma única informação por prioridade: (1) quantidade de PC com entrega naquele dia; (2) se não houver, a data de entrega mais antiga de PC aberto no formato dd/mm/aaaa - quantidade (soma dos PCs dessa data); (3) senão “Pré Compra” (Ag Pag aberta); (4) senão “Solicitação de Compra”; (5) senão 0. Células com PC/Pré Compra/SC são clicáveis e abrem o detalhe com todas as linhas abertas do item. Esses avisos não entram no cálculo da coluna Falta — a falta continua usando só saldo, consumo e entrada numérica do dia.',
  },
  {
    id: 'motivos',
    titulo: 'Concluir',
    ...
    comoLe:
      'Ao lado de Fechar use Pendentes / Concluídos / Todos (filtro local do modal, combinável com os filtros Excel). Corrija datas anteriores a hoje — elas permanecem editáveis mesmo com a linha já verde; ao informar produção anterior à entrega, a entrega é atualizada automaticamente para a mesma data (também nas ações de lote do pedido). Preencha motivo e o toggle Previsão confiável (Não / meio / Sim — sempre inicia no meio; Sim ou Não obrigatório para concluir). Fundo verde = linha ok. Carradas ROTA com vários pedidos aparecem desdobradas por item. Fechar ou Cancelar grava o rascunho e mantém simulação, motivos, observações e confiável; ao reabrir, a grade reflete o estado mais recente (alterações feitas fora do modal prevalecem). Limpeza completa só ocorre após concluir com sucesso ou ao fechar a visualização do sequenciamento. Com motivos preenchidos, você pode anexar um PDF assinado (obrigatório se algum for não abonado; opcional nos demais). Um arquivo vale para todo o lote e fica no histórico de cada item. Itens em que só o Confiável mudou (previsão continua a mesma) também são gravados no Gerenciador: a conclusão registra uma confirmação da data vigente com o novo Confiável, sem exigir motivo. Por isso, o selo Confiável/Não confiável do Gerenciador e do calendário acompanha o sequenciamento concluído, e um novo sequenciamento já nasce com esse estado.',
  },
  {
    id: 'financeiro',
