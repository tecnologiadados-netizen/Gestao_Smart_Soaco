...
const SECOES: SecaoAjuda[] = [
  ...
  },
  {
    ...
    comoLe:
      'Use para ver carga diária e gargalos. As quantidades já consideram o estoque “congelado” no momento da simulação — não é o saldo bruto do ERP. Os setores aparecem em ordem alfabética; “(vazio)”, “Não considerar na meta” e “Outros” ficam nas três últimas linhas da grade (antes do Total Geral). Os quatro botões da legenda (⚠️, Confiável, Não confiável e Em branco) alternam somente a exibição do respectivo ícone nas células e nos níveis de drill; não filtram nem alteram números, linhas ou dados do calendário. O filtro TipoF é múltipla escolha (como Pedido), com as opções presentes na coluna TipoF das linhas; vazio ou todas marcadas = Todos, e pode ser combinado com Pedido, Confiável e Ressuprimento. O filtro Ressuprimento (ao lado de Confiável) usa o Método de ressuprimento da aba Geral do produto no Nomus: Comprado, Fabricado, Como padrão fabricado, Como padrão comprado; produto sem o campo entra em “Não informado”. A grade mostra só as linhas cujo PA tem o método marcado. AS e PP seguem essa demanda filtrada (não refiltram a BOM). Em todos os filtros de múltipla escolha, os itens marcados aparecem no topo da lista. O botão “Somente ⚠️” filtra a grade para mostrar só as quantidades com ⚠️ (itens sem data de produção, posicionados pela previsão atual). Os filtros Pedido, TipoF, Confiável e Ressuprimento exibem “Todos” quando nenhuma opção ou todas estão marcadas; “Limpar filtros” aparece só com filtro parcial ou “Somente ⚠️” ativo. Ao fechar o calendário (Esc ou Fechar) e reabrir, Pedido, TipoF, Confiável, Ressuprimento, Somente ⚠️ e a aba Produção/Materiais críticos permanecem como estavam (na mesma aba do navegador; “Limpar filtros” zera e grava o estado limpo). O filtro Confiável restringe a Confiáveis / Não confiáveis / Em branco, com a mesma precedência do rascunho (override → snapshot → em branco). Nas células, no drill e no modal de itens do PD, além do ⚠️ de previsão, aparecem ícones dos estados presentes: todos Confiável → confirmação verde; algum Não confiável → X vermelho; algum Em branco → “?” em caixa; estados mistos mostram todos os ícones presentes. Ao passar o mouse na quantidade da célula, a dica mostra só quatro linhas, nesta ordem: Carradas, Retirada, Entrega G. The e Requisição, com a quantidade de cada uma (a soma coincide com a célula). No drill da qtde do dia: se houver só um TipoF, essa tela é pulada; se esse TipoF tiver só uma carrada, abre direto a lista de pedidos. No breadcrumb, o último nível mostra o código e o nome da carrada em navegação (ex.: “Carrada: RM123 · ROTA …”); clique nesse rótulo para abrir o mesmo modal de detalhe da carrada da grade principal (pedidos, itens e produtos vinculados). O próprio calendário e o modal de itens do PD (ou Reprogramar) são janelas flutuantes: arraste pelo cabeçalho e redimensione pelo canto inferior direito. Ao abrir sozinho, o calendário ocupa aproximadamente 90% da largura e 78% da altura da tela, centralizado com margem para arrastar e redimensionar pelo canto inferior direito; no modal de itens, o botão “Visualizar calendário” (ao lado do aviso de setor) sobe o modal do PD e encaixa o pivô do calendário na metade inferior da tela — ambos permanecem navegáveis; “Ocultar calendário” recentraliza as janelas. Nos modais do calendário, carradas, ajuste e consulta de estoque, o ícone de copiar ao lado de PD, romaneio ou código copia o respectivo número/código. No modal de itens do PD, à direita dos dados do pedido, aparece Observações com o texto livre do pedido no ERP (distinto da rota/carrada). Em rascunho, “Reprogramar” no PD altera só a simulação (produção/previsão no Map sim): a data de produção informada remove o indicador ⚠️ e reposiciona a célula nas novas datas; o Gerenciador e demais módulos só mudam quando o sequenciamento for concluído. “Confirmar esta data” (abaixo da data atual) copia essa data para o campo nova correspondente; a gravação só ocorre ao clicar em Salvar (com motivo/confiável quando a previsão estiver envolvida). Em tipofs especiais (Requisição, Retirada, Entrega Grande Teresina), só os itens marcados no checkbox mudam — não todos os pedidos daquele TipoF. Em tipof carradas (rota ROTA …), ao marcar ao menos um item o sistema confirma e replica as datas a todos os itens de todos os pedidos no mesmo código de romaneio (RM), para datas únicas na carrada.',
  },
  {
    id: 'ressuprimento-calendario',
    titulo: 'Filtro Ressuprimento no calendário',
    oQueE:
      'Recorta os componentes exibidos e calculados nas bolinhas AS e PP pelo Método de ressuprimento da aba Geral do próprio componente no Nomus.',
    comoLe:
      'Ao selecionar Comprado, Fabricado, Como padrão fabricado, Como padrão comprado ou Não informado, os semáforos e os modais AS/PP usam exatamente esse mesmo recorte. As duas tabelas exibem a coluna Método de ressuprimento para auditoria. Se uma data ou célula não tiver nenhum componente no recorte, sua bolinha AS/PP não aparece. Sem filtro de Ressuprimento, a bolinha AS volta ao padrão da produção: aparece em toda célula com quantidade assim que o motor de materiais carrega (verde = ok; vermelho = falta). PP verde = ok; PP vermelho = falta.',
  },
  {
    id: 'semaforo',
    titulo: 'Semáforo de materiais',
    oQueE:
      'Indicador de disponibilidade de materiais (almox secundário + pedidos de compra), com horizonte por material, e bolinha PP à parte para componentes da Recurso 1000. Escopo AS exclui Matéria Prima.',
    comoLe:
      'No rascunho editável e na consulta ao vivo, saldo/PC/BOM do AS vêm do Nomus em tempo real (igual à produção); só snapshot concluído/somente leitura usa a base congelada do Gravar. No cabeçalho de cada data, a bolinha AS resume o dia inteiro (vermelho = há material em falta; verde = ok). Ao lado esquerdo de cada quantidade da grade, a mesma bolinha avalia só aquela célula (setor × data): vermelho se algum material em falta no dia tem consumo daquele setor; verde caso contrário. Clique na bolinha do cabeçalho para ver todos os materiais do dia; na bolinha da célula, o modal filtra pelo setor. Verde/amarelo/vermelho resumem risco no período analisado. No modal Materiais do dia, a coluna Entrada PC mostra uma única informação por prioridade: (1) quantidade de PC com entrega naquele dia; (2) se não houver, a data de entrega mais antiga de PC aberto no formato dd/mm/aaaa - quantidade (soma dos PCs dessa data); (3) senão “Pré Compra”; (4) senão “Solicitação de Compra”; (5) senão 0. Células com PC/Pré Compra/SC são clicáveis e abrem o detalhe com todas as linhas abertas do item. Esses avisos não entram no cálculo da coluna Falta — a falta continua usando só saldo, consumo e entrada numérica do dia. A bolinha PP é independente (não usa PC): lista componentes da Perfiladeira 1000 (recurso Nomus 124) na BOM dos produtos da célula/dia. Células com os dois universos mostram AS e PP lado a lado. Clique em PP abre Código, Descrição (simplificada), Ressuprimento, Estoque, Consumo e Falta. Consumo acumula a demanda do componente no recorte (setor) até aquela data — a soma das origens (carrada × PD, com coluna Data quando há mais de um dia) fecha com a coluna. O estoque parte do snapshot da última Recurso 1000 e é consumido dia a dia (todos os setores no mesmo poço): a coluna Estoque é o saldo no início daquele dia. Falta é a necessidade acumulada (como no AS): máx(0, consumo do dia − saldo início + falta dos dias anteriores); na célula, essa falta é rateada pela participação do setor no consumo acumulado. PP verde = ok; PP vermelho = algum componente com falta naquele recorte. O modal de estoque continua só leitura (PA Nomus + produção da programação) e mostra também o já consumido nos dias anteriores.',
  },
  {
    id: 'motivos',
    titulo: 'Concluir',
    oQueE:
      'Ao concluir, datas/previsões vencidas e mudanças de previsão ficam na mesma grade. A coluna Pedido concentra PD (com copiar), data de emissão, cliente e carrada; Cliente e Carrada deixam de ser colunas visíveis, mas seus filtros Excel ficam nos botões do cabeçalho de Pedido. Código (com copiar), Descrição, datas, Qtde, Motivo e a coluna Confiável / Obs. (toggle compacto “Confiável [Não | meio | Sim]” com o ícone de observação abaixo) completam a linha. Todo item com id de pedido exige motivo. Motivos do calendário e o Confiável efetivo da grade (rascunho ou snapshot) já entram nesse estado.',
    comoLe:
      'Ao lado de Fechar use Pendentes / Concluídos / Todos (filtro local do modal, combinável com os filtros Excel). A grade do modal usa as mesmas datas efetivas da simulação da grade principal (produção e entrega por carrada/item). Corrija datas anteriores a hoje — fundo verde só quando produção e entrega são ≥ hoje, entrega ≥ produção, e motivo/confiável preenchidos quando exigidos. Ao editar produção, a entrega **não** é reduzida automaticamente; só sobe se a nova produção passar da entrega já informada (use os ícones →/← para replicar manualmente). Editar datas **não reordena** a grade: a ordem vigente é mantida e a classificação padrão (data de produção, carrada, número do pedido, descrição) só é reaplicada ao clicar em **Atualizar classificação** (ou ao reabrir o modal). Preencha motivo e o toggle Previsão confiável (Não / meio / Sim). O Confiável herda o valor efetivo da grade de sequenciamento (escolha do rascunho ou do snapshot já exibido na coluna Confiável); fica no meio só quando ainda não há valor efetivo — Sim ou Não é obrigatório para concluir; você pode alterar no modal. Carradas ROTA com vários pedidos aparecem desdobradas por item. Ao confirmar, o sistema grava **produção antes da previsão** no Gerenciador. Fechar ou Cancelar grava o rascunho e mantém simulação, motivos, observações e confiável; ao reabrir, a grade reflete o estado mais recente. Limpeza completa só ocorre após concluir com sucesso ou ao fechar a visualização do sequenciamento. Com motivos preenchidos, você pode anexar um PDF assinado (obrigatório se algum for não abonado; opcional nos demais). Itens em que só o Confiável mudou também são gravados no Gerenciador via confirmação da data vigente — usando a data efetiva da carrada/item; essa confirmação não altera a data e não é bloqueada por divergência com a data de produção do Gerenciador. Carradas em formação ficam fora dessa confirmação (as datas delas não são gerenciadas nesta tela). Pedido baixado no Nomus entre o rascunho e a conclusão (some da lista viva do ERP) não é gravado no Gerenciador e **não impede** concluir o snapshot — o aviso de sucesso indica quantos foram ignorados. Se alguma gravação falhar, a mensagem de erro identifica o PD e a carrada da linha.',
  },
  {
    id: 'financeiro',
