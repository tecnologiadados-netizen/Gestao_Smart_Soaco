...
function CelulaMetaQuantitativa({
  row,
  mes,
  cacheRef,
}: {
  ...
  // Em unidade "pedidos", a grade aponta para os pedidos contabilizados no realizado.
  if (row.unidade === 'pedidos') {
    return (
      <CelulaComDetalhe
        mes={mes}
        setor={row.setor}
  ...
  return <CelulaMetaNiveis row={row}>{label}</CelulaMetaNiveis>;
}

function CelulaMetaNiveis({
  row,
  children,
}: {
  row: PainelProducaoApuracaoRow;
