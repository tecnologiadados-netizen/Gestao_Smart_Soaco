...
import {
  ...
  obterOpcoesFiltroRessupAlmox,
  obterOpcoesFiltroCascataRessupAlmox,
  buscarOpcoesFiltroRessupAlmox,
  filtrosRessupAlmoxPipePayload,
...
import {
  ...
  type RessupRowUserInputs,
} from '../../utils/ressupAlmoxObservacoes';
import { compareRowsBySortLevels, type SortLevel } from '../../hooks/useGradeFiltrosExcel';
import {
  SORT_DEFAULT_RESSUP_ALMOX,
  getOrderLabelsForRessupCol,
  isRessupColData,
  isRessupColNumeric,
  parseDateSortValue,
  sortLevelsEqual,
} from '../../utils/ressupAlmoxGradeSort';
import {
  RESSUP_DEFAULT_COL_WIDTHS,
  clampColWidth,
...
const COL_DEFS = [
  ...
  { key: 'qtdSolicit', label: 'Solicitação' },
  ...
  { key: 'estoqAtual', label: 'Estoque atual' },
  { key: 'qtdeUltComp', label: 'Qtde Ultm Comp' },
  { key: 'ultimoFornecedor', label: 'Último fornecedor' },
...
const COLUNAS_APROVACAO_GRADE = new Set<ColKey>(['qtdAprov', 'dataNecessAprov']);
...
type RowUserInputs = RessupRowUserInputs;
...
function getCmRawFromRow(row: Record<string, unknown>): number {
  const cm = Number(getRowValue(row, ['Consumo Medio', 'consumo medio']) ?? 0);
  return Number.isFinite(cm) ? cm : NaN;
}
...
function aplicadoParaFiltrosPreview(aplicado: {
  ...
  return {
    ...
    diaSemana: splitCsv(aplicado.diaCompra).join('|') || undefined,
...
function getEstoqAtualNumerico(row: Record<string, unknown>): number {
...
/** Saldo projetado = −Qtde Emp + Solicitação (Qtd Liberada) + Estoque atual + PC Pend + Pré Compra */
function calcSaldoProjetado(row: Record<string, unknown>): number | null {
...
function fmtSaldoProjetado(row: Record<string, unknown>): string {
  const n = calcSaldoProjetado(row);
  if (n == null) return '—';
  return n.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
}

/** Remove prefixo CNPJ/código e retorna apenas o nome do fornecedor (mesma regra do mapa de cotação). */
function apenasNomeFornecedor(nomeOuConcat: string | null | undefined): string {
  if (nomeOuConcat == null || !String(nomeOuConcat).trim()) return '';
...
function fmtUltimoFornecedor(row: Record<string, unknown>): string {
  const raw = String(
    getRowValue(row, ['Ultimo Fornecedor', 'ultimo fornecedor', 'ultimoFornecedor']) ?? ''
  ).trim();
  if (!raw) return '—';
  return apenasNomeFornecedor(raw) || raw;
}

/** Cobertura = Saldo projetado ÷ CM (CM = 0 usa 0,01 no divisor). */
function calcCobertura(row: Record<string, unknown>): number | null {
  const saldo = calcSaldoProjetado(row);
  if (saldo == null) return null;
  let cm = Number(getRowValue(row, ['Consumo Medio', 'consumo medio']) ?? 0);
  if (!Number.isFinite(cm)) return null;
  if (cm === 0) cm = 0.01;
  return saldo / cm;
}

function fmtCobertura(row: Record<string, unknown>): string {
  const n = calcCobertura(row);
...
function getRessupCell(
  ...
  switch (key) {
    ...
    case 'ultimoFornecedor':
      return fmtUltimoFornecedor(row);
    case 'dataUltEntrada':
...
function valueForSort(
  ...
  if (s === '—') return '';
  if (isRessupColData(key)) return parseDateSortValue(s);
  const forNum = s.replace(/\s/g, '').replace(/R\$\s?/i, '').replace(/\./g, '').replace(',', '.');
  const n = Number(forNum);
  if (isRessupColNumeric(key) || (Number.isFinite(n) && /[\d]/.test(s))) return n;
...
export default function RessupAlmoxAnalisePage() {
  ...
  const [filterDiaCompra, setFilterDiaCompra] = useState('');
  const filtrosRef = useRef({
    codigo: filterCodigo,
    descricao: filterDescricao,
    coleta: filterColeta,
    diaCompra: filterDiaCompra,
  });
  filtrosRef.current = {
    codigo: filterCodigo,
    descricao: filterDescricao,
  ...
  const [sortLevels, setSortLevels] = useState<SortLevel[]>(() => [...SORT_DEFAULT_RESSUP_ALMOX]);
  ...
  const [detalheConsultaModal, setDetalheConsultaModal] = useState<{
    tipo: 'saldo' | 'solicitacao' | 'cotacao';
    idProduto: number;
    codigo: string;
    descricao: string;
  } | null>(null);
  ...
  const pcPendCacheRef = useRef(new Map<number, RessupAlmoxPcPendLinha[]>());
  const empenhoCacheRef = useRef(new Map<string, RessupEmpenhoPedidoResultado>());
  const saldoCacheRef = useRef(new Map<number, SaldoSetorDetalhe[]>());
  ...
  const cascataDeps = useMemo(
    () => [filterCodigo, filterDescricao, filterColeta, filterDiaCompra].join('\u0001'),
    [filterCodigo, filterDescricao, filterColeta, filterDiaCompra]
  );
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      void obterOpcoesFiltroCascataRessupAlmox(
        filtrosRessupAlmoxPipePayload(f.codigo, f.descricao, f.coleta, f.diaCompra)
      ).then((r) => {
        if (!r.error) {
  ...
  const buscarCodigoFiltroAsync = useCallback(async (term: string) => {
    const f = filtrosRef.current;
    const r = await buscarOpcoesFiltroRessupAlmox(
      'codigo',
      term,
      filtrosRessupAlmoxPipePayload(f.codigo, f.descricao, f.coleta, f.diaCompra)
    );
  ...
  const levelsOrdenacao = useMemo((): SortLevel[] => {
    ...
    return SORT_DEFAULT_RESSUP_ALMOX;
  ...
  const temFiltrosGrade =
    ...
    sortState != null ||
    !sortLevelsEqual(sortLevels, SORT_DEFAULT_RESSUP_ALMOX) ||
  ...
  const limparFiltrosGrade = () => {
    ...
    setSortState(null);
    setSortLevels([...SORT_DEFAULT_RESSUP_ALMOX]);
    setColunaFiltroAberta(null);
    setFiltroAbertoRect(null);
  };
  ...
  const onColResizePointerDown = useCallback((colKey: ColKey, e: React.PointerEvent<HTMLSpanElement>) => {
    ...
    colResizeRef.current = {
      colKey,
      startX: e.clientX,
      startW: colWidths[colKey] ?? RESSUP_DEFAULT_COL_WIDTHS[colKey] ?? 96,
    };
  }, [colWidths]);
   ...
   */
  const coletasFiltro = opcoesFiltro.coletas;
  const diasCompraFiltro = opcoesFiltro.diasSemana;

  const handleFiltrar = () => {
    const temCodigo = filterCodigo.trim() !== '';
    const temDescricao = filterDescricao.trim() !== '';
    const temColeta = filterColeta.trim() !== '';
    const temDiaCompra = filterDiaCompra.trim() !== '';
    if (!temCodigo && !temDescricao && !temColeta && !temDiaCompra) {
      setMsgFiltro(
        'Informe ao menos um filtro: Código do produto, Descrição do produto, Nome da coleta ou Dia da compra.'
      );
      return;
    }
    setMsgFiltro(null);
  ...
  const executarFiltrar = async (req: boolean) => {
    ...
    const diaCompraValues = filterDiaCompra.split('|').filter(Boolean);
    const aplicadoLocal = {
      codigo: codigoValues.join(', '),
      descricao: descricaoValues.join(', '),
      coleta: coletaValues.join(', '),
      diaCompra: diaCompraValues.join(', '),
    ...
    try {
      const r = await listarRessupAlmoxRegistroPreview({
        ...
        diaSemana: diaCompraValues.join('|') || undefined,
        considerarRequisicoes: req,
      });
  ...
  const resumoFiltros =
    aplicado == null
      ...
      : [
          ...
          aplicado.diaCompra && `Dia da compra: ${aplicado.diaCompra}`,
        ]
          .filter(Boolean)
          .join(' · ');
  ...
  const abrirDetalheHistorico = useCallback(async (id: number) => {
    ...
    try {
      ...
      const aplicadoPayload = {
        ...
        diaCompra: payload.aplicado?.diaCompra ?? '',
      ...
      if (payload.aplicado) {
        try {
          const preview = await listarRessupAlmoxRegistroPreview(aplicadoParaFiltrosPreview(aplicadoPayload));
          if (req !== detalheHistoricoReqRef.current) return;
      ...
      if (colWidthsUi) {
        const merged = { ...readRessupColWidths() };
  ...
  const larguraMinimaTabela = useMemo(() => {
    ...
    for (const col of colunasVisiveisLista) {
      w += colWidths[col.key] ?? RESSUP_DEFAULT_COL_WIDTHS[col.key] ?? 96;
    }
  ...
  const carregarDetalheConsultaModal = useCallback(async (): Promise<{ error?: string }> => {
    ...
    if (tipo === 'saldo') {
      const cached = saldoCacheRef.current.get(idProduto);
      if (cached) {
        setDetalheSaldo(cached);
        return {};
      }
      const r = await obterSaldoDetalhe(idProduto);
      if (!r.error) saldoCacheRef.current.set(idProduto, r.data);
  ...
  const overlayPrincipalAtivo =
    loading || gravandoAnalise || salvandoAlteracoes || processandoAnalise || concluindoAnalise || historicoDetalheCarregando;
    ...
    <div className="relative flex flex-col flex-1 min-h-0 p-3 max-w-[1920px] mx-auto w-full">
        ...
        {filtrosPopoverAberto && (
          ...
          >
            ...
            >
              ...
              <div className="grid min-w-0 grid-cols-1 items-end gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div className="min-w-0">
                ...
                <MultiSelectWithSearch
                  label="Dia da compra"
                  placeholder="Todos"
                  options={diasCompraFiltro}
                  value={filterDiaCompra}
                  onChange={(v) => startTransition(() => setFilterDiaCompra(v))}
                  labelClass={labelClass}
                  inputClass={inputClass}
                  optionLabel="dias"
                  valueSeparator="|"
      ...
      <div className="flex flex-col flex-1 min-h-0 gap-1.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {mostrarGradeAnalise && linhas.length > 0 && (
              <div className="flex items-center gap-1.5">
                <label
                  htmlFor="filtro-compra-recorrente-ressup"
                  className="text-xs font-medium text-slate-600 dark:text-slate-300 shrink-0"
                >
                  Compra recorrente
                </label>
                <select
                  id="filtro-compra-recorrente-ressup"
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="flex items-center justify-between gap-2 border-b border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-600 dark:border-slate-600 dark:bg-slate-900/50 dark:text-slate-300">
            <span>
              Empenho — requisições de loja:{' '}
            ...
            {linhasOrdenadas.length > 0 && (
              <span className="tabular-nums text-slate-500 dark:text-slate-400">
                {linhasOrdenadas.length} produto{linhasOrdenadas.length === 1 ? '' : 's'}
              </span>
            )}
          </div>
          <div
            ...
            >
              <colgroup>
                {colunasVisiveisLista.map((col) => (
                  <col
                    key={col.key}
                    style={{ width: colWidths[col.key] ?? RESSUP_DEFAULT_COL_WIDTHS[col.key] ?? 96 }}
                  />
                ))}
              </colgroup>
              <thead>
                  ...
                  {colunasVisiveisLista.map((col) => {
                    ...
                    >
                      <div className="flex min-w-0 items-start justify-between gap-1">
                        <span className="min-w-0 flex-1 truncate leading-tight text-[11px] sm:text-xs" title={col.key !== 'qtdeEmp' ? col.label : undefined}>
                          {col.key === 'qtdeEmp' ? (
                            <RotuloComDica rotulo={col.label} dica={DICA_EMPENHO_LIQ_GRADE} headerClaro />
                          ) : (
                            col.label
                          )}
                        </span>
              ...
              <tbody className="text-slate-700 dark:text-slate-200 divide-y divide-slate-200 dark:divide-slate-600">
                ...
                {!loading &&
                  linhasOrdenadas.map((row, idx) => {
                    ...
                    const coberturaVal = calcCobertura(row);
                    const cmRaw = getCmRawFromRow(row);
                    const coberturaBaixa =
                      Number.isFinite(cmRaw) &&
                      cmRaw > 0.5 &&
                      coberturaVal != null &&
                    ...
                    return (
                      ...
                      >
                        {colunasVisiveisLista.map((col) => {
                              ...
                              >
                                <div className="flex min-w-0 items-center gap-1">
                                  ...
                                  <input
                                    ...
                                    data-rowkey={rowKey}
                                    data-colkey={col.key}
                                    className="ressup-almox-num-input min-w-0 flex-1 rounded border border-slate-300 bg-white px-1.5 py-1 text-right text-xs text-slate-800 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-100"
                                    placeholder="—"
                          ...
                          if (col.key === 'ultimoFornecedor') {
                            const val = getRessupCell(row, 'ultimoFornecedor', inputs);
                            return (
                              <td
                                key={col.key}
                              ...
                              >
                                <span className="block max-w-full truncate">{val}</span>
                              </td>
                            );
                          }
                          if (col.key === 'cobertura') {
                            const cob = calcCobertura(row);
                                  ...
                                  <GradeCelulaModalBtn
                                    onClick={() =>
                                      setEmpenhoModal({
                                        ...
                                        codigo: getRessupCell(row, 'codigo', inputs),
                                        descricao: getRessupCell(row, 'descricao', inputs),
                                        saldoAtual: getEstoqAtualNumerico(row),
                                      })
                                    }
                                    title="Ver empenho por produto acabado e venda direta"
                                  >
                                    {val}
                                  </GradeCelulaModalBtn>
                                ) : (
      ...
      {colunaFiltroAberta && filtroAbertoRect && (
        <GradeFiltroExcelPortal
          ...
          sortAscLabel={getOrderLabelsForRessupCol(colunaFiltroAberta).asc}
          sortDescLabel={getOrderLabelsForRessupCol(colunaFiltroAberta).desc}
          showNumericFilters={isRessupColNumeric(colunaFiltroAberta)}
          onSortAsc={(colId) => {
      ...
      <ModalClassificarGrade
        ...
        initialLevels={sortLevels.length > 0 ? sortLevels : SORT_DEFAULT_RESSUP_ALMOX}
        getOrderLabels={getOrderLabelsForRessupCol}
        onApply={(levels) => {
      ...
      {detalheConsultaModal && (
        <ModalConsultaEstoqueDetalhe
          open
          titulo={
            detalheConsultaModal.tipo === 'saldo'
              ? `Estoque atual — ${detalheConsultaModal.codigo}`
        ...
        >
          {({ carregando, erro }) => {
            ...
            if (detalheConsultaModal.tipo === 'saldo') {
