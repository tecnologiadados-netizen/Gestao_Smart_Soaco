...
export default function ModalDashEntregasLeadTimeTipoF({
  open,
  titulo,
  subtitulo,
  onClose,
  ...
  const [dados, setDados] = useState<TipoFLeadTimeResumo[]>([]);
  ...
  useRegisterModalEscape({
    id: 'dash-entregas-leadtime-tipof',
    onClose,
    zIndex: 12900,
    enabled: open,
  });
  ...
  const maxDias = Math.max(...dados.map((d) => d.leadTimeMedioDias), 1);
  ...
  return createPortal(
    ...
    >
      <div
        className="flex max-h-[min(85vh,640px)] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-600 dark:bg-slate-900"
        role="dialog"
        aria-modal
              ...
              {dados.map((d) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-[72px] text-right text-xs tabular-nums text-slate-500">
                      {formatNumero(d.quantidade)} linhas
                    </span>
                  </button>
                );
              })}
