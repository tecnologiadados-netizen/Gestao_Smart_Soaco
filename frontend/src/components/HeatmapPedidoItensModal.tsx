    ...
    >
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto overscroll-contain px-4 py-3">
          {ordenados.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-500">Nenhum item encontrado para este pedido.</p>
          ) : (
            <table className="w-full border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50 text-left dark:border-slate-600 dark:bg-slate-900/50">
                  ...
                  <th className="py-2 text-right font-semibold text-slate-700 dark:text-slate-200">Valor</th>
                </tr>
              </thead>
              <tbody className="text-slate-700 dark:text-slate-200">
                {ordenados.map((row, i) => (
                  <tr key={`${row.codigo}-${i}`} className="border-b border-slate-100 dark:border-slate-700">
                    ...
                    <td className="py-1.5 text-right tabular-nums">{formatarValor(row.valorPendente ?? 0)}</td>
                  </tr>
                ))}
