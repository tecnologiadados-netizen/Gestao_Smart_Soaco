import { useRef, memo, useState, useMemo, useEffect, useCallback, useLayoutEffect } from "react";
import { useVirtualizer } from "@tanstack/react-virtual";
import { Filter, Search, Pencil, MessageSquare, Minus } from "lucide-react";
...
const ROW_PX = 42;
const ACTION_COL_PX = 116;
const MIN_COL_PX = 56;
const MAX_COL_PX = 560;
const COL_WIDTHS_LS_KEY = "faltas-ausencias-col-widths-v2";

/** Larguras iniciais (px) por campo — ajuste fino para leitura (CID, nomes, etc.). */
const DEFAULT_COL_PIXELS: Partial<Record<keyof FaltaRow, number>> = {
  data: 118,
  mesFalta: 108,
  matricula: 100,
  nomeFuncionario: 228,
  ...
  tipo: 152,
  cid: 280,
  localAtendimento: 200,
  medicoResponsavel: 200,
  observacoes: 220,
};

type Col = { key: keyof FaltaRow; label: string; listId?: string };
...
type Props = {
  columns: Col[];
  /** Linhas já filtradas por período; usadas para montar a lista de valores únicos do filtro. */
  sourceRows: FaltaRow[];
  rows: FaltaRow[];
  canEdit?: boolean;
  onEditRow: (id: FaltaRow["id"]) => void;
  maxHeightClass?: string;
  columnFilters: Partial<Record<keyof FaltaRow, FaltaColumnFilter>>;
  onColumnFilterApply: (key: keyof FaltaRow, filter: FaltaColumnFilter) => void;
  sortConfig: { key: keyof FaltaRow; dir: "asc" | "desc" } | null;
  onSortChange: (next: { key: keyof FaltaRow; dir: "asc" | "desc" } | null) => void;
  selectedRowIds?: Set<string>;
  onToggleRowSelection?: (id: FaltaRow["id"]) => void;
  onToggleAllVisibleSelection?: () => void;
  allVisibleSelected?: boolean;
  someVisibleSelected?: boolean;
  disableSelection?: boolean;
  ausenciaAttachments?: Map<string, AusenciaLaunchAttachment>;
};

function displayCellText(row: FaltaRow, key: keyof FaltaRow): string {
  const v = row[key];
  if (key === "data") {
...
function ColumnHeaderExcelFilter({
  ...
  col: Col;
  sourceRows: FaltaRow[];
  committed: FaltaColumnFilter | undefined;
  onApply: (f: FaltaColumnFilter) => void;
  sortConfig: Props["sortConfig"];
  onSortChange: Props["onSortChange"];
}) {
  const [open, setOpen] = useState(false);
  const isDateColumn = col.key === "data";
  const uniques = useMemo(() => columnUniqueValues(sourceRows, col.key), [sourceRows, col.key]);
  ...
  const visible = useMemo(() => {
    ...
    return uniques.filter((v) => textIncludesSearch(displayCellFilterLabel(v), search));
  }, [uniques, search]);
...
function FaltaGridRowInner({
  ...
  isSelected: boolean;
  onToggleSelection?: (id: FaltaRow["id"]) => void;
  disableSelection?: boolean;
  attachment?: AusenciaLaunchAttachment;
}) {
  const zebra = rowIdx % 2 === 0 ? "bg-card" : "bg-muted/20";
  const obsTrim = String(row.observacoes ?? "").trim();
  const obsDecoded = decodeAusenciaSuspensaoObservacoes(obsTrim);
  const rowGrid = (
    <div
      className={cn(
    ...
    >
            ...
            <div className="flex items-center gap-1 min-w-0 w-full px-2">
              {obsTrim ? (
                <MessageSquare
                  ...
                  aria-label="Há observações nesta ausência"
                />
              ) : null}
              <span className="truncate text-sm tabular-nums">{displayCellText(row, col.key)}</span>
            </div>
          ) : (
            <span className="truncate w-full px-2 text-sm">{displayCellText(row, col.key)}</span>
          )}
        </div>
      ))}
      <div className="flex items-center justify-center gap-0.5 border-l border-border bg-inherit shrink-0 px-0.5">
        <Checkbox
          ...
          aria-label="Selecionar linha para exclusão em lote"
          className="mr-0.5"
        />
        ...
        <div className="px-4 py-3.5 max-h-[min(65vh,26rem)] overflow-y-auto overflow-x-hidden rounded-[inherit]">
          ...
          {obsDecoded ? (
            <div className="space-y-2 text-base leading-relaxed text-foreground">
              {obsDecoded.tipoCadastro.trim() ? (
                <p className="text-sm">
                  <span className="text-muted-foreground">Tipo de sanção (cadastro):</span>{" "}
                  {obsDecoded.tipoCadastro.trim()}
                </p>
              ) : null}
              <p className="whitespace-pre-wrap">{obsDecoded.motivo.trim() || "—"}</p>
            </div>
          ) : (
            <p className="text-base leading-relaxed whitespace-pre-wrap text-foreground">{obsTrim}</p>
          )}
        </div>
      </TooltipContent>
    </Tooltip>
...
const FaltaGridRow = memo(FaltaGridRowInner);

export function FaltaAusenciasVirtualGrid({
  columns,
  sourceRows,
  rows,
  canEdit = true,
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return;
    return attachDialogSafeWheelScroll(el, "both");
  }, []);
      ...
      >
        <div className="w-max min-w-full" style={{ minWidth: gridMinWidth }}>
          ...
          >
            ...
            <div
              className="border-l border-border flex items-center justify-center gap-1 shrink-0 bg-muted/95"
            ...
            >
              ...
              />
              {someVisibleSelected && !allVisibleSelected ? <Minus className="h-3.5 w-3.5 text-muted-foreground" /> : null}
            </div>
          </div>
          {emptyBody ? (
                ...
                return (
                  ...
                  >
                    <FaltaGridRow
                      ...
                      canEdit={canEdit}
                      onEditRow={onEditRow}
                      isSelected={Boolean(selectedRowIds?.has(String(row.id)))}
                      onToggleSelection={onToggleRowSelection}
                      disableSelection={disableSelection}
                      attachment={ausenciaAttachments?.get(String(row.id))}
                    />
                  </div>
                );
              })}
