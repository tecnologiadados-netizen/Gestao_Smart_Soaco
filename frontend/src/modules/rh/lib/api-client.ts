...
export async function getOrganicoRepresentantesAtivos(): Promise<OrganicoRepresentante[]> {
  ...
  if (isRhApiConfigured() && !isLocalhost) {
    try {
  ...
  if (isLocalhost) {
    const devRes = await fetch("/__nomus/representantes", {
      method: "GET",
      headers: { Accept: "application/json" },
    });
    if (!devRes.ok) {
      let msg = `Representantes (dev proxy): ${devRes.status}`;
      try {
        const j = (await devRes.json()) as { error?: string };
    ...
    const raw = (await devRes.json()) as { representantes?: OrganicoRepresentante[] };
    return (Array.isArray(raw?.representantes) ? raw.representantes : [])
      .map((item) => normalizeOrganicoRepresentante(item))
      .filter((rep) => !isOrganicoRepresentanteExcluido(rep));
  }
