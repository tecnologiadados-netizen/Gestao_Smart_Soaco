...
import { AvaliacaoFornecedorConsultaPanel } from "@/components/registros/avaliacao-fornecedor-consulta-panel";
import { CodigoDocumentoCell } from "@/components/registros/codigo-documento-cell";
import {
  TABLE_FILTER_ALL,
  TableFilterField,
  TableFilterSearch,
...
import { RegistroDetalheDialog } from "@/components/registros/registro-detalhe-dialog";
...
function novoRegistroHref(tipoFiltro: string): string {
  if (isModuloRegistroTipo(tipoFiltro)) {
    return `/registros?tipo=${tipoFiltro}`;
  }
  return "/registros";
}
...
export default function RegistrosConsultaContent() {
  ...
  function atualizarTipoFiltro(value: string) {
    ...
    router.replace(`/registros/consulta?tipo=${value}`);
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link href={novoRegistroHref(tipoFiltro)}>
