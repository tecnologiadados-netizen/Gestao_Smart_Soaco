...
import { resolveTaskAssignee } from "@/lib/documents/task-assignee";
import {
...
import { taskTypeLabels } from "@/lib/utils/status-labels";
import {
  getTaskActionHref,
  getTaskActionLabel,
} from "@/lib/documents/task-routes";
        ...
        {items.length === 0 ? (
          ...
          items.map(({ doc, situacao }) => (
            ...
            >
              ...
              <div className="mt-4">
                <Link
                  href={`/documentos?revalidar=${doc.id}`}
                  className={cn(
                    buttonVariants({ size: "sm" }),
                    "h-8 gap-1.5 text-xs"
                  )}
