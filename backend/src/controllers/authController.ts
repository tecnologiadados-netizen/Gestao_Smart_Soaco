...
import { seedUserActivity } from '../middleware/inactivityLogout.js';
  ...
  try {
    ...
    let usuario:
      | {
          ...
          grupo?: { ativo: boolean; logoutInatividadeMinutos: number | null } | null;
        }
      | null = null;
    try {
      usuario = await prisma.usuario.findUnique({
        ...
        select: {
          ...
          grupo: { select: { ativo: true, logoutInatividadeMinutos: true } },
        },
      });
    } catch (dbErr) {
    ...
    try {
      ...
      console.error('[auth] Erro ao registrar atividade no login:', (activityErr as Error)?.message ?? activityErr);
    }
    seedUserActivity(usuario.login, usuario.id, usuario.grupo?.logoutInatividadeMinutos ?? null, agora.getTime());
