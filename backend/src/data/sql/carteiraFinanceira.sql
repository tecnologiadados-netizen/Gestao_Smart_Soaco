SELECT
	...
	CASE
		...
		AND me.opcao = 'Retirada na Só Móveis') THEN '2-Retirada na So Moveis'
		WHEN (de.observacoes IS NULL
		AND me.opcao = 'Retirada na Só Aço') THEN '1-Retirada na So Aço'
		WHEN (de.observacoes IS NULL
		AND IFNULL(m.nome, mc.nome) = 'Teresina'
		AND aloreq.opcao = 'Sim') THEN '5-Requisicao'
		WHEN (de.observacoes IS NULL
		AND (IFNULL(m.nome, mc.nome) IN ('Timon', 'Teresina', 'Nazaria', 'Demerval Lobão', 'Curralinhos'))
		AND aloreq.opcao = 'Não') THEN '3-Entrega em Grande Teresina'
	...
	CASE
		WHEN (CASE
			...
			WHEN (de.observacoes IS NULL) THEN '4-Inserir em Romaneio'
			ELSE de.observacoes
		END) REGEXP 'Retirada|Entrega' THEN 'PI'
	...
	CASE
		WHEN (CASE
			...
			WHEN (de.observacoes IS NULL) THEN '4-Inserir em Romaneio'
			ELSE de.observacoes
		END) REGEXP 'Retirada|Entrega' THEN 'Teresina'
	...
	SUM(
		(ROUND(ip.valorTotalComDesconto * IFNULL(t.aliquotaIPI / 100, 0), 2) + IFNULL(ip.valorTotalComDesconto, 0))
		* (
			CASE
				WHEN de.id IS NULL THEN
					(ip.qtde - IFNULL(trm.totalRomaneadoItem, 0)) / ip.qtde
	...
	case
		when 
        (adt.valorAdiantamento / MAX(totped.valorTotalPedido)) > 0.99 then 0
        
		when adt.valorAdiantamento is null then
...
FROM
	itempedido ip
LEFT JOIN pedido pd ON
	pd.id = ip.idPedido
LEFT JOIN tipopedido tpd ON
...
LEFT JOIN (
	...
	FROM
		itempedido ip2
	LEFT JOIN tributacao t2 ON
		t2.idItemPedido = ip2.id
	WHERE
		ip2.status <> 6
	GROUP BY
		ip2.idPedido
) totped ON
	totped.idPedido = pd.id
LEFT JOIN (
	SELECT
		idItemPedido,
		SUM(qtdeVinculada) AS totalRomaneadoItem
	FROM
		itempedidoromaneio
	GROUP BY
		idItemPedido
) trm ON
	trm.idItemPedido = ip.id
LEFT JOIN (
	SELECT
		pd.id,
		aloreq.opcao
	FROM
		pedido pd
	LEFT JOIN atributopedidovalor apvreq ON
		apvreq.idPedido = pd.id
	LEFT JOIN atributolistaopcao aloreq ON
		aloreq.id = apvreq.idListaOpcao
	WHERE
		apvreq.idAtributo = 313
) aloreq ON
	aloreq.id = pd.id
LEFT JOIN condicaopagamento cp ON
	cp.id = pd.idCondicaoPagamento
LEFT JOIN endereco ed ON
	ed.id = pd.idEnderecoLocalEntrega
LEFT JOIN municipio m ON
	ed.idMunicipio = m.id
LEFT JOIN municipio mc ON
	mc.id = pe.idMunicipio
LEFT JOIN (
	SELECT
		idItemPedido,
		idRomaneio,
		SUM(qtdeVinculada) AS qtdeVinculada
	FROM
		itempedidoromaneio
	GROUP BY
		idItemPedido,
	...
	prm.idItemPedido = ip.id
LEFT JOIN documentoestoque de ON
	de.id = prm.idRomaneio
LEFT JOIN formapagamento fp ON
	fp.id = pd.idFormaPagamento
LEFT JOIN (
	SELECT
	...
	WHERE
		de_nf.idTipoMovimentacao IN (48, 82, 44, 150)
			AND nfe_nf.status IN (2, 4)
		GROUP BY
			ideipv.idItemPedidoVenda
) nfef ON
	nfef.idItemPedidoVenda = ip.id
LEFT JOIN (
	SELECT
		ideipv.idItemPedidoVenda,
	...
	ef_base.idItemPedidoVenda = ip.id
LEFT JOIN (
	SELECT
		p.id,
...
LEFT JOIN (
	SELECT
		...
		alo.opcao
	FROM
		atributopedidovalor apev
	LEFT JOIN atributolistaopcao alo ON
		alo.id = apev.idListaOpcao
	WHERE
		apev.idAtributo = 592
) emp ON
	emp.idPedido = pd.id
LEFT JOIN pessoa vr ON
	vr.id = COALESCE(pd.idVendedor, pd.idRepresentante)
LEFT JOIN (
	SELECT
	...
	LEFT JOIN itemdocumentoestoque_itempedidovenda ideipv ON
		ideipv.idItemDocumentoEstoque = ide.idItemOrigemDevolucao
	LEFT JOIN itempedido ip ON
		ip.id = ideipv.idItemPedidoVenda
