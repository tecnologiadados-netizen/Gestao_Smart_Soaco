...
then 'Teresina' else
	...
	ifnull(max(ef_base.datasBaseEF), '') as 'Data base entrega futura',
...
END AS StatusPedido
	...
	group by ideipv.idItemPedidoVenda) nfef on nfef.idItemPedidoVenda = ip.id
	left join
	(select
	ideipv.idItemPedidoVenda,
	GROUP_CONCAT(
	  DISTINCT DATE_FORMAT(de_nf.dataBaseParcelas, '%d/%m/%Y')
	  ORDER BY de_nf.dataBaseParcelas
	  SEPARATOR ', '
	) as datasBaseEF
	from itemdocumentoestoque_itempedidovenda ideipv
	left join itemdocumentoestoque ide on ide.id = ideipv.idItemDocumentoEstoque
	...
	group by ideipv.idItemPedidoVenda) ef_base on ef_base.idItemPedidoVenda = ip.id
	left join
	(select
...
group by ip.id
	...
	WHERE ip.status NOT IN (2,3)
  AND UPPER(TRIM(pd.nome)) = UPPER(TRIM(/*PD_ENCERRADOS_FILTER*/))
