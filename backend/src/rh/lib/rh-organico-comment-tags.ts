export const RH_ORGANICO_COMMENT_TAGS_CONFIG_KEY = 'organico_comment_tags_catalog';

export const RH_ORGANICO_COMMENT_TAG_OPTIONS = [
...
export function normalizeRhCommentTagId(tagId: string): string {
  return normalizeKnownTagId(tagId);
}

export function normalizeRhCommentTagCatalog(input: unknown): RhOrganicoCommentTagOption[] {
  if (!Array.isArray(input)) {
    return [...RH_ORGANICO_COMMENT_TAG_OPTIONS];
  }

  const unique = new Map<string, RhOrganicoCommentTagOption>();
  ...
  return unique.size > 0 ? Array.from(unique.values()) : [...RH_ORGANICO_COMMENT_TAG_OPTIONS];
}

export function parseRhCommentTagCatalog(raw: string | null | undefined): RhOrganicoCommentTagOption[] {
  if (!raw?.trim()) return [...RH_ORGANICO_COMMENT_TAG_OPTIONS];
  try {
    return normalizeRhCommentTagCatalog(JSON.parse(raw));
  } catch {
    return [...RH_ORGANICO_COMMENT_TAG_OPTIONS];
  }
}

export function buildRhCommentTagAccess(
  optionsOrDefaultValue: RhOrganicoCommentTagOption[] | boolean = [...RH_ORGANICO_COMMENT_TAG_OPTIONS],
  defaultValue = true,
): Record<string, boolean> {
  const options = Array.isArray(optionsOrDefaultValue) ? optionsOrDefaultValue : [...RH_ORGANICO_COMMENT_TAG_OPTIONS];
...
export function buildRhCommentVisibilityAccess(defaultValue = true): Record<RhOrganicoCommentVisibilityId, boolean> {
